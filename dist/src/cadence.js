...
export class RepeatCadence {
     ...
     */
    press(key, nowMs, mods = []) {
        this.stop();
        const emit = { op: 'emit-key', key, mods };
        if (!this.keys.repeatable(key))
            return [emit];
        this.held = key;
     ...
     */
    tick(nowMs, mods = []) {
        if (this.held === null || this.nextFireMs === null)
            return [];
        if (nowMs < this.nextFireMs)
            return [{ op: 'schedule-tick', atMs: this.nextFireMs }];
        const key = this.held;
